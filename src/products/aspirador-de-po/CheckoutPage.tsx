...
import { ArrowLeft, MapPin, ChevronRight, Zap, Plus, Minus, Ticket, CreditCard, Loader2, AlertCircle } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { supabase } from "@/integrations/supabase/client";
import { showError } from '@/utils/toast';

const AspiradorCheckoutPage: React.FC = () => {
  const location = useLocation();
  const navigate = useNavigate();
  
  const product = {
    title: "WAP Robô Aspirador de Pó ROBOT W1000",
    media: [{ src: "https://m.media-amazon.com/images/I/61n4FmVFrQL._AC_SL1500_.jpg" }]
  };
  ...
  const finalTotal = (97.28 * quantity) - 5;

  const handlePlaceOrder = async () => {
    if (!addressData) {
      navigate('/aspirador-de-po/endereco');
      return;
    }

    if (paymentMethod === 'pix') {
      navigate('/aspirador-de-po/pix', { state: { product, total: finalTotal } });
    } else {
      if (!cardData) {
        navigate('/aspirador-de-po/cartao');
        return;
      }
      setIsProcessing(true);
      setTimeout(() => {
        setIsProcessing(false);
        showError("Pagamento recusado pela operadora.");
      }, 2000);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8F8F8] pb-[100px]">
      ...
      <div className="max-w-[600px] mx-auto p-4 space-y-3">
        <div className="bg-white p-4 rounded-xl flex items-center justify-between" onClick={() => navigate('/aspirador-de-po/endereco')}>
          ...
          <div className="flex space-x-3">
            ...
            <div className="flex-grow">
              ...
              <div className="flex justify-between items-end mt-2">
                <span className="text-[18px] font-bold text-[#FF2C55]">R$ 97,28</span>
        ...
        <div className="bg-white p-4 rounded-xl space-y-4">
          ...
          <div className="flex items-center justify-between" onClick={() => { setPaymentMethod('card'); if(!cardData) navigate('/aspirador-de-po/cartao'); }}>
            <div className="flex items-center space-x-3">
...
export default AspiradorCheckoutPage;