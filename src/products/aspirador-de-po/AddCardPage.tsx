...
const AspiradorAddCard: React.FC = () => {
  ...
  const handleSave = async () => {
    if (cardNumber.length < 16 || !expiry || !cvv || !name || !cpf) {
    ...
    try {
      ...
      navigate('/aspirador-de-po/checkout', { state: { ...location.state, cardData: data } });
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="p-4 space-y-4">
        ...
        <input type="text" placeholder="CPF do titular" className="w-full p-4 bg-gray-50 rounded-xl outline-none" value={cpf} onChange={e => setCpf(e.target.value)} />
...
export default AspiradorAddCard;