  ...
  return (
    <div className="min-h-screen bg-[#F8F8F8] flex flex-col items-center">
      <div className="w-full max-w-[600px] bg-white border-b h-12 flex items-center px-4 sticky top-0 z-50">
        <button onClick={() => navigate(-1)} className="p-1"><ArrowLeft size={24} className="text-gray-900" /></button>
        <div className="flex-grow flex justify-center"><img src="https://i.ibb.co/TD1mSYdD/tiktok-1.png" alt="TikTok" className="h-6" /></div>
      ...
      <main className="w-full max-w-[600px] p-4 space-y-4">
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          <div className="flex items-center space-x-3 mb-4">
            <div className="w-12 h-12 rounded-lg bg-gray-50 flex-shrink-0 border p-1"><img src={product.media[0].src} className="w-full h-full object-contain" /></div>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 text-center">
          {loading ? (
            <div className="flex flex-col items-center py-8"><Loader2 className="animate-spin text-[#FF2C55] mb-3" size={32} /></div>
          ) : paymentApproved ? (
            <div className="py-8 space-y-3 animate-in fade-in zoom-in duration-500">
              <div className="w-16 h-16 bg-[#00BFA5] rounded-full flex items-center justify-center mx-auto mb-4"><Check size={32} className="text-white" /></div>
            ...
            <div className="space-y-5">
              <Button onClick={handleCopy} className={cn("w-full h-12 rounded-full font-bold text-[15px]", copied ? "bg-[#00BFA5]" : "bg-[#FF2C55]")}>{copied ? "Código Copiado!" : "Copiar Código PIX"}</Button>
            </div>
          )}
        </div>
      </main>
