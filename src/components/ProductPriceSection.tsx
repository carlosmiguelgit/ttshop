  ...
  return (
    <div className="bg-white">
      <div className="relative h-[64px] flex items-center justify-between px-4 text-white overflow-hidden">
        <img src="/faixalaranja.png" alt="BG" className="absolute inset-0 w-full h-full object-cover" />
        <div className="relative z-10 flex flex-col justify-center">
          <div className="flex items-center space-x-1.5">
            ...
            <span className="text-3xl font-bold leading-none tracking-tight">
              R$ {product.currentPrice.split(',')[0]}<span className="text-xl">,{product.currentPrice.split(',')[1]}</span>
            </span>
          </div>
        ...
        <div className="relative z-10 flex flex-col items-end justify-center">
          <div className="flex items-center text-[13px] font-bold"><CustomZapIcon className="w-4 h-4 mr-1" />Oferta Relâmpago</div>
          <div className="text-[11px] mt-1">Termina em <span className="font-bold">{formatTime(seconds)}</span></div>
        </div>
      </div>

      <div className="p-4 space-y-3.5">
        <div className="flex items-center text-[11px] text-gray-800">
          <CreditCard size={15} className="text-gray-600 mr-1.5" />
          <span>2x R$ {(parseFloat(product.currentPrice.replace(',', '.')) / 2).toFixed(2).replace('.', ',')} <span className="text-[#FF2C55] font-medium ml-1">sem juros</span></span>
        ...
        <div className="flex items-center space-x-2 text-[12px]">
          <div className="flex items-center space-x-0.5">
            ...
            <span className="text-[#4A90E2] font-medium ml-0.5">({stats.reviews})</span>
          </div>
          <span className="text-gray-200">|</span>
          <span className="text-gray-500 font-medium">{(stats.sales / 1000).toFixed(1)}K vendidos</span>
        </div>

        <div className="flex justify-between items-center py-3 border-t border-gray-100 cursor-pointer" onClick={onShippingClick}>
          <div className="flex items-start space-x-3">
            <div className="mt-0.5 text-gray-600">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M10 17h4M2 9h4M2 12h3M2 15h2" opacity="0.5" /><path d="M7 17a2 2 0 1 0 4 0 2 2 0 1 0-4 0ZM17 17a2 2 0 1 0 4 0 2 2 0 1 0-4 0Z" /><path d="M13 18h4M5 17h2M13 5H1v12h4m12 0h4v-7l-3-3h-4V5Z" />
              </svg>
            </div>
            <div className="flex flex-col">
              ...
              <span className="text-[11px] text-gray-500 mt-1">Taxa de envio: <span className="line-through">R$ 9,18</span></span>
            </div>
          </div>
