...
const ProductActionsBar: React.FC<ProductActionsBarProps> = ({ onAddToCartClick, onBuyWithCouponClick }) => {
  ...
  const AddToCartButton = (
    <Button 
      variant="outline" 
      className={cn(
        "bg-gray-100 text-gray-900 border-none rounded-xl h-14 text-base font-bold hover:bg-gray-200",
        "flex-grow w-full max-w-[150px] whitespace-normal leading-snug py-2" // Ajustes para tamanho e quebra de linha
      )}
  ...
  const BuyNowButton = (
    <Button 
      className={cn(
        "bg-[#FF3366] text-white rounded-xl h-14 text-lg font-bold hover:bg-[#E02E5C]",
        "flex-grow w-full max-w-[150px] whitespace-nowrap" // Ajustes para tamanho e cor
      )}
